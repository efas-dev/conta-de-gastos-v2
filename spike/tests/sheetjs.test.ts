...
import { candidateSheetjs } from '../src/candidate-sheetjs.js'
...
const OUTPUT_PATH = path.resolve(import.meta.dirname, '../output/sheetjs-output.xlsx')
const REPORT_PATH = path.resolve(import.meta.dirname, '../output/sheetjs-report.json')
const VIRGEM_PARTS_DIR = path.resolve(import.meta.dirname, '../fixtures/virgem-parts')

describe('candidata sheetjs', () => {
  beforeAll(async () => {
    fs.mkdirSync(path.dirname(OUTPUT_PATH), { recursive: true })
    // Se candidateSheetjs lançar, beforeAll falha → C1 = fail
    await candidateSheetjs(MODELO_PATH, OUTPUT_PATH, REPORT_PATH, dataset, VIRGEM_PARTS_DIR)
  })
  ...
  it('C4 — ref da Tabela1 registrado no relatório (pass ou fail são achados válidos)', () => {
    // SheetJS Community 0.18.5 descarta xl/tables/table1.xml ao reescrever.
    // O candidato registra C4='fail' como achado; o teste verifica que o
    // critério foi avaliado (não impõe que passe — comportamento idêntico ao C7).
    const report = JSON.parse(fs.readFileSync(REPORT_PATH, 'utf-8'))
    expect(['pass', 'fail']).toContain(report.C4)
  })
