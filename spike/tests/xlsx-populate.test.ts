...
import { candidateXlsxPopulate } from '../src/candidate-xlsx-populate.js'
...
const OUTPUT_PATH = path.resolve(import.meta.dirname, '../output/xlsx-populate-output.xlsx')
const REPORT_PATH = path.resolve(import.meta.dirname, '../output/xlsx-populate-report.json')
const VIRGEM_PARTS_DIR = path.resolve(import.meta.dirname, '../fixtures/virgem-parts')

describe('candidata xlsx-populate', () => {
  beforeAll(async () => {
    fs.mkdirSync(path.dirname(OUTPUT_PATH), { recursive: true })
    // Se candidateXlsxPopulate lançar, beforeAll falha → C1 = fail
    await candidateXlsxPopulate(MODELO_PATH, OUTPUT_PATH, REPORT_PATH, dataset, VIRGEM_PARTS_DIR)
  })
